...
function AuthContent() {
  ...
  const onRegister = async (data: RegisterForm) => {
    ...
    try {
      ...
      // Fire Google Ads signup conversion tracking
  ...
  const transitionToView = (view: 'landing' | 'login' | 'register') => {
    ...
    setTimeout(() => {
      ...
      if (view === 'register') { setRegisterPassword(''); setRegisterStep(1); }
          ...
          <div className="w-full bg-white/95 backdrop-blur-sm rounded-2xl shadow-xl p-6">
            ...
            <div className="text-center pb-4">
              ...
              <div className="flex items-center justify-center gap-2 mt-3">
                <div className={`h-1.5 rounded-full transition-all duration-300 ${registerStep === 1 ? 'w-8 bg-[#8B6F47]' : 'w-4 bg-[#E8DCC8]'}`} />
                <div className={`h-1.5 rounded-full transition-all duration-300 ${registerStep === 2 ? 'w-8 bg-[#8B6F47]' : 'w-4 bg-[#E8DCC8]'}`} />
              </div>
            </div>
            ...
            {registerStep === 1 ? (
              ...
              <form onSubmit={registerForm.handleSubmit(onRegister)} className="space-y-3">
                ...
                <button
                  ...
                  className="w-full text-sm text-[#8B6F47] font-medium hover:underline py-2"
                >
                  ← Back
                </button>

                <p className="text-xs text-center text-[#A67A5B]/50">
