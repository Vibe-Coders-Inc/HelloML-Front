...
function AnimatedSection({ children, className = '', delay = 0 }: { children: React.ReactNode; className?: string; delay?: number }) {
  ...
  return (
    <motion.section ref={ref} initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay }} className={className}>
      {children}
    </motion.section>
  );
}
...
function VoiceWaveform() {
  return (
    <div className="flex items-center justify-center gap-[3px] h-8 my-6">
      {[0, 0.15, 0.3, 0.45, 0.6, 0.75, 0.9, 1.05, 1.2, 1.05, 0.9, 0.75, 0.6].map((d, i) => (
        <motion.div
          key={i}
          className="w-[3px] rounded-full bg-[#8B6F47]/40"
          animate={{ height: ['8px', `${12 + Math.sin(i * 0.8) * 14}px`, '8px'] }}
          transition={{ duration: 1.8 + i * 0.1, repeat: Infinity, ease: 'easeInOut', delay: d }}
        />
      ))}
    </div>
  );
...
/* ─── Use Case Carousel ─── */
const useCases = [
  { persona: '🔨 Contractor', text: 'On the job site — your agent books the estimate.' },
  { persona: '💇 Stylist', text: 'Mid-appointment — your agent reschedules the no-show.' },
  { persona: '⚖️ Attorney', text: 'In court — your agent takes the message.' },
  { persona: '🏢 Owner', text: "Can't be everywhere — your agent can." },
];
...
function UseCaseCarousel() {
  ...
  return (
    <div className="relative h-40 sm:h-36 max-w-lg mx-auto">
      {useCases.map((uc, i) => (
        <motion.div
          key={i}
        ...
        >
          <span className="text-3xl mb-3">{uc.persona}</span>
          <p className="text-lg sm:text-xl text-[#8B7355] font-medium">{uc.text}</p>
        </motion.div>
      ))}
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 flex gap-2">
        {useCases.map((_, i) => (
          <button key={i} onClick={() => setActive(i)} className={`w-2 h-2 rounded-full transition-colors ${i === active ? 'bg-[#8B6F47]' : 'bg-[#E8DCC8]'}`} />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F3] overflow-x-hidden">
      ...
      <section className="min-h-[80vh] md:min-h-screen flex flex-col items-center justify-center pt-24 md:pt-40">
        <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="max-w-4xl mx-auto text-center px-6">
          <motion.h1 variants={staggerItem} className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-bold text-[#8B6F47] tracking-tight mb-6">
            AI that <span style={{ fontFamily: 'Borel, cursive' }}>answers</span> your phone.
          </motion.h1>
          <motion.p variants={staggerItem} className="text-base sm:text-lg md:text-2xl text-[#8B7355] mb-2 max-w-2xl mx-auto leading-relaxed px-2">
      ...
      {/* ───────── 2. DASHBOARD PREVIEW ───────── */}
      <motion.div ref={dashboardRef} style={{ y: dashY }} className="w-full max-w-5xl mx-auto px-4 md:px-6 mb-16 md:mb-24">
      ...
      {/* ───────── 3. SOCIAL PROOF BAR ───────── */}
      <AnimatedSection className="text-center pt-4 pb-10 md:pt-6 md:pb-16">
        <p className="text-[10px] sm:text-xs uppercase tracking-[0.2em] text-[#A67A5B]/70 font-medium mb-6 md:mb-8">Built by engineers from</p>
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="flex items-center justify-center gap-10 sm:gap-14 md:gap-20">
          <motion.div variants={staggerItem}>
          ...
          <motion.div variants={staggerItem} className="flex flex-col items-center opacity-50">
      ...
      {/* ───────── 4. HOW IT WORKS ───────── */}
      <AnimatedSection className="py-16 md:py-32 px-4 md:px-6 bg-gradient-to-b from-[#FAF8F3] to-[#F5EFE6]">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-[#8B6F47] mb-4">How it works</h2>
          <p className="text-base md:text-lg text-[#8B7355] mb-16">Three steps. Five minutes. Done.</p>
          <HowItWorksSteps />
        </div>
      </AnimatedSection>

      {/* ───────── 5. LIVE DEMO CTA ───────── */}
      <AnimatedSection className="py-20 md:py-36 px-4 md:px-6 relative overflow-hidden">
        <div className="max-w-3xl mx-auto text-center relative z-10">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-[#8B6F47] mb-4">Hear it for yourself.</h2>
          <p className="text-lg text-[#8B7355] mb-10 max-w-md mx-auto">Call our AI agent and experience what your customers will hear.</p>
          {/* Phone mockup */}
          <div className="relative inline-block mb-10">
            <div className="w-56 h-96 sm:w-64 sm:h-[28rem] rounded-[2.5rem] border-4 border-[#8B6F47]/20 bg-white/40 backdrop-blur-sm flex flex-col items-center justify-center relative overflow-hidden">
              <div className="absolute top-4 w-20 h-1.5 rounded-full bg-[#8B6F47]/15" />
              <div className="text-5xl mb-4">📞</div>
              <p className="text-sm text-[#8B7355] font-medium mb-6">HelloML AI Agent</p>
              <Link href="/demo">
                <Button className="relative bg-[#8B6F47] hover:bg-[#A67A5B] text-white rounded-full px-6 py-4 text-base font-medium shadow-lg group z-10">
                  Call Our AI <ArrowRight className="ml-2 w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </Button>
              </Link>
              {/* Pulsing rings */}
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="absolute w-32 h-32 rounded-full border border-[#8B6F47]/10 animate-pulse-ring" />
                <div className="absolute w-48 h-48 rounded-full border border-[#8B6F47]/5 animate-pulse-ring-delayed" />
              </div>
            </div>
          </div>
        </div>
      ...
      <AnimatedSection className="py-16 md:py-32 px-4 md:px-6">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-[#8B6F47] mb-4 text-center">Everything you need.</h2>
          <p className="text-base md:text-lg text-[#8B7355] mb-12 text-center">One agent. Fully loaded.</p>
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} variants={staggerContainer} className="grid sm:grid-cols-2 gap-6 md:gap-8">
            {[
              { icon: '📅', title: 'Books appointments on the spot.', desc: 'Your agent checks availability and creates bookings directly on your calendar. No back-and-forth.' },
              { icon: '📄', title: 'Answers from your documents.', desc: 'Upload FAQs, policies, or menus. Your agent searches them in real time to answer caller questions.' },
              { icon: '📝', title: 'Transcripts after every call.', desc: 'Get a full transcription and summary delivered to your dashboard the moment a call ends.' },
              { icon: '📞', title: 'One number. Always on.', desc: 'We provision a dedicated phone number for your business. Your agent picks up 24/7.' },
            ].map((f, i) => (
              <motion.div key={i} variants={staggerItem} className="group p-6 md:p-8 rounded-2xl bg-white/50 border border-[#E8DCC8]/50 hover:border-[#8B6F47]/20 hover:-translate-y-1 hover:shadow-xl hover:shadow-[#8B6F47]/8 transition-all duration-300 will-change-transform">
      ...
      <AnimatedSection className="py-16 md:py-32 px-4 md:px-6 bg-gradient-to-b from-[#F5EFE6] to-[#FAF8F3]">
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-[#8B6F47] mb-4">Built for people who are <span style={{ fontFamily: 'Borel, cursive' }}>busy</span>.</h2>
          <p className="text-base md:text-lg text-[#8B7355] mb-12">Your agent works while you work.</p>
          <UseCaseCarousel />
        </div>
      </AnimatedSection>
      ...
      <AnimatedSection className="py-16 md:py-24 px-4 md:px-6">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-base md:text-lg text-[#8B7355] mb-12">Connects to your calendar, docs, and files automatically.</p>
          <IntegrationMarquee />
        </div>
      </AnimatedSection>
      ...
      <AnimatedSection className="py-16 md:py-32 px-4 md:px-6">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-[#8B6F47] mb-4">Simple pricing. No surprises.</h2>
          <p className="text-lg text-[#8B7355] mb-10">One plan. Everything included.</p>
          <div className="max-w-sm mx-auto bg-white/40 backdrop-blur-xl border border-[#E8DCC8]/60 rounded-3xl p-8 shadow-lg shadow-[#8B6F47]/5 hover:shadow-2xl hover:shadow-[#8B6F47]/10 hover:border-[#8B6F47]/20 transition-all duration-500 mb-8">
      ...
      <AnimatedSection className="py-16 md:py-32 px-4 md:px-6 relative overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto text-center relative z-10">
          <motion.h2 initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-[#8B6F47] mb-8">
          ...
          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.4 }} className="mt-6 text-[#8B7355]">No credit card required</motion.p>
        </div>
      </AnimatedSection>
...
function HowItWorksSteps() {
  ...
  const steps = [
    ...
    ), title: 'Your AI answers calls', desc: '24/7. On your dedicated number. Instantly.' },
  ];

  return (
    <div ref={ref} className="grid sm:grid-cols-3 gap-8 md:gap-12">
      {steps.map((step, i) => (
        <motion.div
          key={i}
        ...
        >
          <div className="w-20 h-20 rounded-2xl bg-white/60 border border-[#E8DCC8]/50 flex items-center justify-center mb-4 shadow-md">
            {step.icon}
          </div>
